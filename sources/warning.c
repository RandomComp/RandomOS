...
int8* getWarningDescription(enum WarningsType warningType) {
	if (warningType == 0) return "What? One more time. I don't understand what you're mumbling. \n\nMy friend taught me how to talk to my USB port, so now I have to mess with it for days without understanding it...\n\n";

	else if (warningType == 1) return "Dude, everything is booked, what can we do, we'll come tomorrow :(\n\n";
