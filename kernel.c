...
void main(uint32 magic, struct multibootInfo* bootInfoArg) {
	...
	for (uint16 i = 0; i < columns - (pos + length); i++) {
		UGSMASCIIputString(" ");
	}
	...
	RMALcompileASCII("push ebp mov ebp, esp mov eax, [ ebp +8 ] mov ebx, [ ebp +12 ] label1: add eax, ebx jmp label1 pop ebp ret");
	...
	tempForegroundColor = foregroundColor;
				...
				if (bufferLength > 0 && checkLength != 0) {
					...
					if (bFinded) {
						...
						if (command == HELP) {
							...
							uint32 tempForegroundColor2 = foregroundColor;

							uint32 tempBackgroundColor2 = backgroundColor;
							...
							foregroundColor = tempForegroundColor2;

							backgroundColor = tempBackgroundColor2;
						}
					}
