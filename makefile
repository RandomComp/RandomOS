...
image:
	...
	@losetup /dev/loop8 ./hdd.img
	@(echo c; echo u; echo n; echo p; echo 1; echo ;  echo ; echo a; echo 1; echo t; echo c; echo w;) | fdisk /dev/loop8 1>/dev/null 2>&1 || true

	@echo "Mounting partition to /dev/loop9..."
	@losetup /dev/loop9 ./hdd.img \
		--offset 32256 \
		--sizelimit 8224768
	@losetup -d /dev/loop8
...
all: gassources nasmsources csources kernel.bin image clean run
run:
...
gassources:
	...
	@as -o sources/mems.o sources/mem.s

	@as -o sources/setjmp.o sources/setjmp.s
